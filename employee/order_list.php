<?php 
  ...
  $stmt=$pdo->prepare("SELECT * FROM sale , customer , sale_detail , book WHERE sale.c_id = customer.c_id AND sale_detail.o_id = sale.s_id AND sale_detail.b_id = book.b_id ORDER BY sale_detail.sale_id");
  $stmt->execute();
    ...
    <main>
      <div class="container-fluid" style="background-color: #ffffff; border-radius: 20px; padding: 20px;box-shadow: 0px 0px 25px 10px #1e272e; margin-top: 50px; padding: 50px; margin-bottom: 100px;">
        <div class="form-group  text-center ">
          <label class="text-primary" style="font-size: 26px;"> รายการสั่งซื้อทั้งหมด </label>
        </div><br>
        <div class="table-responsive text-nowrap">
          <table class="table table-hover">
          <thead class="text-primary  text-center">
            <tr>
              <th style="font-size: 20px;">รหัสสั่งซื้อ</th>
              <th style="font-size: 20px;">ชื่อ</th>
              <th style="font-size: 20px;">นามสกุล</th>
              <th style="font-size: 20px;">ที่อยู่</th>
              <th style="font-size: 20px;">จังหวัด</th>
              <th style="font-size: 20px;">ประเทศ</th>
              <th style="font-size: 20px;">รหัสไปรษณีย์</th>
              <th style="font-size: 20px;">เบอร์โทร</th>
              <td style="font-size: 20px;">ชื่อสินค้า</td>
              <td style="font-size: 20px;">ราคา</td>
              <td style="font-size: 20px;">จำนวน</td>
              <td style="font-size: 20px;">ราคารวม</td>
            </tr>
          </thead>
          <tbody class=" text-center">
            <?php 
              while ($order_list = $stmt->fetch()) { 
                 $Total = 0;
                 $Total = $order_list["qty"] * $order_list["b_price"];
                ?>

                <tr>
                  <td style="font-size: 18px;"> <?=$order_list['s_id']?> </td>
                  <td style="font-size: 18px;"> <?=$order_list['c_name']?> </td>
                  <td style="font-size: 18px;"> <?=$order_list['c_surname']?> </td>
                  <td style="font-size: 18px;"> <?=$order_list['c_address']?> </td>
                  <td style="font-size: 18px;"> <?=$order_list['c_district']?> </td>
                  <td style="font-size: 18px;"> <?=$order_list['c_province']?> </td>
                  <td style="font-size: 18px;"> <?=$order_list['c_postcode']?> </td>
                  <td style="font-size: 18px;"> <?=$order_list['c_tel']?> </td>
                  <td style="font-size: 18px;"> <?=$order_list['b_name']?> </td>
                  <td style="font-size: 18px;"> <?=$order_list['b_price']?> </td>
                  <td style="font-size: 18px;"> <?=$order_list['qty']?> </td>
