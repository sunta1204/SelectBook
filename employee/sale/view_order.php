<?php 
  ...
  $stmt=$pdo->prepare("SELECT * FROM sale WHERE s_id = ?");
  $stmt->bindParam(1,$_GET['sale_id']);
  $stmt->execute();
  $saleList = $stmt->fetch();
  ...
  $stmt3=$pdo->prepare("SELECT * FROM customer WHERE c_id = ?");
  $stmt3->bindParam(1,$saleList['c_id']);
  $stmt3->execute();
  $customer_order = $stmt3->fetch();
?>
  ...
  <body class="fixed-sn pink-skin" style="background-color: #eeeeee;">
    ...
    <main>
      <div class="container-fluid" style="background-color: #ffffff; border-radius: 20px; padding: 20px;box-shadow: 0px 0px 25px 10px #1e272e; margin-top: 50px; padding: 50px;">
        <div class="form-group  text-center ">
          <label class="text-primary" style="font-size: 26px;"> รายละเอียดการสั่งซื้อ </label>
        </div><br>
        <div class="table-responsive text-nowrap">
          ...
          <thead class="text-primary  text-center">
            <tr>
              ...
              <th style="font-size: 24px;">เบอร์โทร</th>
            </tr>
          </thead>
          <tbody class=" text-center">
            <tr>
              <td style="font-size: 18px;"> <?=$saleList['s_id']?> </td>
              <td style="font-size: 18px;"> <?=$customer_order['c_name']?> </td>
              <td style="font-size: 18px;"> <?=$customer_order['c_surname']?> </td>
              <td style="font-size: 18px;"> <?=$customer_order['c_address']?> </td>
              <td style="font-size: 18px;"> <?=$customer_order['c_district']?> </td>
              <td style="font-size: 18px;"> <?=$customer_order['c_province']?> </td>
              <td style="font-size: 18px;"> <?=$customer_order['c_postcode']?> </td>
              <td style="font-size: 18px;"> <?=$customer_order['c_tel']?> </td>
            </tr>
          </tbody>
        </table><br>
      </div>    
    ...
    <div class="container-fluid" style="background-color: #ffffff; border-radius: 20px; padding: 20px;box-shadow: 0px 0px 25px 10px #1e272e; margin-top: 50px; padding: 50px;margin-bottom: 100px;"> 
      <div class="form-group text-center">
        <label class="text-primary" style="font-size: 26px;"> รายการสั่งซื้อ </label>
      </div><br>
      <div class="table-responsive text-nowrap">
        <table class="table table-hover">
          <thead class="text-primary  text-center">
            <tr>
              <td style="font-size: 24px;">รหัสสินค้า</td>
              <td style="font-size: 24px;">ชื่อสินค้า</td>
              <td style="font-size: 24px;">ราคา</td>
              <td style="font-size: 24px;">จำนวน</td>
              <td style="font-size: 24px;">ราคารวม</td>
            </tr>
          </thead>
          <tbody class="text-center">
            <?php
              ...
              $stmt4=$pdo->prepare("SELECT * FROM sale_detail WHERE o_id = ?");
              $stmt4->bindParam(1,$_GET['sale_id']);
              $stmt4->execute();

              while($order_list = $stmt4->fetch())
              {
                  $stmt5=$pdo->prepare("SELECT * FROM book WHERE b_id = ?");
                  $stmt5->bindParam(1,$order_list['b_id']);
                  $stmt5->execute();
                  $objResult3 = $stmt5->fetch();
                  $Total = $order_list["qty"] * $objResult3["b_price"];
                  $SumTotal = $SumTotal + $Total;
                  ?>
                  <tr>
                    <td style="font-size: 18px;"><?php echo $order_list["b_id"];?></td>
                    <td style="font-size: 18px;"><?php echo $objResult3["b_name"];?></td>
                    <td style="font-size: 18px;"><?php echo $objResult3["b_price"];?> บาท</td>
                    <td style="font-size: 18px;"><?php echo $order_list["qty"];?></td>
                    <td style="font-size: 18px;"><?php echo number_format($Total,2);?> บาท</td>
                  </tr>
                  <?php
               }
                ?>
      ...
      <div class="form-group">
        <label class="text-primary" style="font-size: 30px;"> ราคารวมทั้งหมด : <?php echo number_format($SumTotal,2);?> บาท</label>
      </div>
    </div>
    </main>
