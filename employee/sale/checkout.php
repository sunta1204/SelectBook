<?php 
  ...
  $stmt3=$pdo->prepare("SELECT * FROM customer");
  $stmt3->execute();
...
?>
  ...
  <body class="fixed-sn pink-skin" style="background-color: #eeeeee;">
    ...
    <div class="container-fluid" style="background-color: #ffffff; border-radius: 20px; padding: 20px;box-shadow: 0px 0px 25px 10px #1e272e; margin-top: 50px; padding: 50px;margin-bottom: 100px;"> 
      <div class="form-group text-center">
        <label class="text-primary" style="font-size: 26px;"> ผู้สั่งซื้อ </label>
      </div><br>
      <form action="save_checkout.php" method="post">
        <div class="form-group ">
          <div class="md-form ">
            <select name="c_id" class="browser-default custom-select">
                <option value="" disabled=""> เลือกลูกค้า </option>
              <?php 
                while ($customerList = $stmt3->fetch()) { ?>
                  <option value="<?=$customerList['c_id']?>"> <?=$customerList['c_name']?> </option>
                <?php }
              ?>
            </select>
          </div>
